...
import { type AuthenticatedRequest, authMiddleware } from "../auth/auth.router.js";
import {
  getListsForUser,
...
listsRouter.post("/", async (req: AuthenticatedRequest, res): Promise<void> => {
  try {
    ...
    const list = await createList(req.user!.userId, uuidv4(), name.trim(), Math.floor(Date.now() / 1000));
    res.json(list);
  } catch (err) {
...
listsRouter.patch("/:id", async (req: AuthenticatedRequest, res): Promise<void> => {
  try {
    const { name } = req.body;
    if (!name || typeof name !== "string") {
      res.status(400).json({ error: "name required" });
    ...
    const updated = await renameList(req.user!.userId, req.params.id as string, name.trim());
    if (!updated) {
...
listsRouter.delete("/:id", async (req: AuthenticatedRequest, res): Promise<void> => {
  try {
...
listsRouter.post("/:id/items", async (req: AuthenticatedRequest, res): Promise<void> => {
  try {
    ...
    await upsertTickerStock({ ticker: normalizedTicker, name: normalizedTicker });
...
listsRouter.delete("/:id/items/:ticker", async (req: AuthenticatedRequest, res): Promise<void> => {
  try {
    const owner = await getListOwner(req.params.id as string);
    if (owner !== req.user!.userId) {
      res.status(403).json({ error: "Forbidden" });
