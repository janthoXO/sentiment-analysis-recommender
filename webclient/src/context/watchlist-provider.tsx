...
export function WatchlistProvider({ children }: { children: ReactNode }) {
  ...
  const [tickerResults, setTickerResults] = useState<Record<string, TickerResult>>({});
      ...
      try {
        ...
        if (data.type === "TICKER_UPDATE") {
          ...
          setTickerResults((prev) => {
            ...
            if (
              prevResult &&
              Math.abs(fresh.avgScore - prevResult.avgScore) >= DIVERGENCE_THRESHOLD
            ) {
  ...
  const createList = useCallback(async (name: string): Promise<List | null> => {
    try {
  ...
  const addToList = useCallback(async (listId: string, ticker: string) => {
    try {
      await postApiListsIdItems(listId, { ticker }, { headers: authHeaders() });
      setLists((prev) =>
        prev.map((l) =>
  ...
  const removeFromList = useCallback(async (listId: string, ticker: string) => {
    try {
      await deleteApiListsIdItemsTicker(listId, ticker, { headers: authHeaders() });
      setLists((prev) =>
        prev.map((l) =>
          l.id === listId
  ...
  const listsContainingTicker = useCallback(
    (ticker: string) => lists.filter((l) => l.items.some((i) => i.ticker === ticker)),
    [lists]
  );
