...
export default function StockDetailPage() {
  ...
  const [data, setData] = useState<TickerResult | null>(location.state?.tickerResult || null)
    ...
    <div className="flex flex-col gap-6 max-w-3xl">
      <div className="flex justify-between items-start gap-4">
        <div>
          ...
          <p className="text-xl text-muted-foreground mt-1">{ticker}</p>
        </div>
        <div className="flex flex-col items-end gap-3">
          <Badge
            variant="outline"
            className={cn("text-base font-bold px-4 py-2", overall.className)}
          >
      ...
      <div className="flex flex-col gap-4">
        <h2 className="text-2xl font-semibold">
          News &amp; Articles <span className="text-muted-foreground text-base font-normal">({data.sources.length})</span>
        </h2>
        <div className="flex flex-col gap-4">
          {data.sources.map((source, i) => {
            ...
            return (
              <div key={i} className="p-4 border rounded-xl flex flex-col gap-2">
                <div className="flex justify-between items-start gap-4">
                  ...
                  <Badge
                    variant="outline"
                    className={cn("shrink-0 font-bold", articleSentiment.className)}
                  >
